...
```mermaid
graph TD
    bcs_framework["BCS 超导理论框架"]:::setting
    migdal_eliashberg["Migdal-Eliashberg 框架"]:::setting
    adiabatic_approx["绝热近似与 Migdal 定理 (0.88)"]:::premise
    electron_gas_model["均匀电子气模型 (0.90)"]:::orphan
    main_question["第一性原理计算 Coulomb 赝势"]:::question
...
📌 `adiabatic_approx`   |   Prior: 0.95   |   Belief: **0.88**
...
📌 `electron_gas_model`   |   Prior: 0.90   |   Belief: **0.90**
...
```mermaid
graph TD
    adiabatic_approx["绝热近似与 Migdal 定理 (0.88)"]:::external
    electron_phonon_action["电子-声子作用量分解 (0.84)"]:::premise
    bse_kernel_decomposition["BSE 积分核分解 (0.81)"]:::premise
    precursory_cooper_flow["前驱 Cooper 流方法 (0.79)"]:::derived
...
📌 `electron_phonon_action`   |   Prior: 0.90   |   Belief: **0.84**
...
📌 `bse_kernel_decomposition`   |   Prior: 0.88   |   Belief: **0.81**
...
📌 `precursory_cooper_flow`   |   Belief: **0.79**
...
🔗 **deduction**([BSE 积分核分解](#bse_kernel_decomposition), [电子-声子作用量分解](#electron_phonon_action), [绝热近似与 Migdal 定理](#adiabatic_approx))

<details><summary>Reasoning</summary>

作用量的严格分解保证了电子-声子耦合的正确处理，BSE 积分核的 Coulomb/声子分离在绝热近似下成立，由此严格推导出反常顶点函数的普适对数标度律。
...
graph TD
    adiabatic_approx["绝热近似与 Migdal 定理 (0.88)"]:::external
    pair_propagator_decomposition["配对传播子分解 (0.82)"]:::premise
    cross_term_suppressed["交叉项被等离激元压制 (0.73)"]:::premise
    downfolded_bse["降标 Fermi 面 BSE (0.75)"]:::derived
    pseudopotential_scale_relation["赝势标度关系 (0.92)"]:::orphan
    mu_vdiagmc_values["vDiagMC 赝势计算结果 (0.45)"]:::derived
    rpa_predicts_negative_mu["RPA 预测负赝势 (0.28)"]:::orphan
    downfolding_validated["Downfolding 数值验证 (0.88)"]:::orphan
    downfolding_limitations["Downfolding 适用性限制 (0.90)"]:::orphan
    mu_rpa_vdiagmc_contradiction["mu_rpa_vdiagmc_contradiction (1.00)"]:::orphan
    ueg_vertex_challenge["电子气顶点计算挑战 (0.88)"]:::external
    homotopic_expansion["同伦展开消除对数发散 (0.78)"]:::external
    vdiagmc_method["变分图形蒙特卡洛方法 (0.83)"]:::external
...
📌 `pair_propagator_decomposition`   |   Prior: 0.88   |   Belief: **0.82**
...
📌 `cross_term_suppressed`   |   Prior: 0.82   |   Belief: **0.73**
...
🔗 **deduction**([配对传播子分解](#pair_propagator_decomposition), [交叉项被等离激元压制](#cross_term_suppressed), [绝热近似与 Migdal 定理](#adiabatic_approx))

<details><summary>Reasoning</summary>

配对传播子的相干/非相干分解提供了降标的数学基础，交叉项被 $\omega_c^2/\omega_p^2$ 压制保证了 Coulomb 和声子通道可独立处理，两者结合在绝热近似下严格推导出仅依赖频率的 Fermi 面 BSE。
...
📌 `pseudopotential_scale_relation`   |   Prior: 0.92   |   Belief: **0.92**
...
🔗 **noisy_and**([变分图形蒙特卡洛方法](#vdiagmc_method), [同伦展开消除对数发散](#homotopic_expansion), [电子气顶点计算挑战](#ueg_vertex_challenge))

<details><summary>Reasoning</summary>

在均匀电子气上用 VDiagMC 计算四点顶点函数，同伦展开解决低温收敛问题，BTS 标度关系保证尺度一致性，共同得到 $\mu_{E_F}$ 的精确值。
...
📌 `rpa_predicts_negative_mu`   |   Prior: 0.50   |   Belief: **0.28**
...
📌 `downfolding_validated`   |   Prior: 0.88   |   Belief: **0.88**
...
📌 `downfolding_limitations`   |   Prior: 0.90   |   Belief: **0.90**
...
📌 `mu_rpa_vdiagmc_contradiction`   |   Belief: **1.00**
...
```mermaid
graph TD
    ueg_vertex_challenge["电子气顶点计算挑战 (0.88)"]:::premise
    homotopic_expansion["同伦展开消除对数发散 (0.78)"]:::premise
    vdiagmc_method["变分图形蒙特卡洛方法 (0.83)"]:::premise
...
📌 `ueg_vertex_challenge`   |   Prior: 0.92   |   Belief: **0.88**
...
📌 `homotopic_expansion`   |   Prior: 0.85   |   Belief: **0.78**
...
📌 `vdiagmc_method`   |   Prior: 0.88   |   Belief: **0.83**
...
```mermaid
graph TD
    individual_corrections_large["各项多体修正均很大 (0.90)"]:::premise
    corrections_cancel["多体修正近精确抵消 (0.88)"]:::premise
    dfpt_validated_numerically["EFT 与 DFPT 数值一致 (0.88)"]:::premise
    dfpt_reliable_for_simple_metals["DFPT 对简单金属可靠 (0.63)"]:::derived
    ward_identity_hypothesis["Ward 恒等式解释抵消 (0.70)"]:::derived
...
📌 `individual_corrections_large`   |   Prior: 0.90   |   Belief: **0.90**
...
📌 `corrections_cancel`   |   Prior: 0.88   |   Belief: **0.88**
...
📌 `dfpt_validated_numerically`   |   Prior: 0.88   |   Belief: **0.88**
...
📌 `dfpt_reliable_for_simple_metals`   |   Belief: **0.63**
...
🔗 **noisy_and**([各项多体修正均很大](#individual_corrections_large), [多体修正近精确抵消](#corrections_cancel), [EFT 与 DFPT 数值一致](#dfpt_validated_numerically))

<details><summary>Reasoning</summary>

各项多体修正单独很大，但在有效耦合中几乎精确抵消，并通过 VDiagMC 的逐点数值验证确认 → DFPT 对简单金属可靠。
...
📌 `ward_identity_hypothesis`   |   Prior: 0.70   |   Belief: **0.70**

> Ward 恒等式和电子-声子顶点的规范不变性强制要求自能、屏蔽和顶点修正在长波极限下系统性地互相抵消。DFPT 中各项大修正的近精确抵消不是数值巧合，而是守恒律约束多体修正在物理可观测量中组合方式的必然结果。

🔗 **abduction**([多体修正近精确抵消](#corrections_cancel))

<details><summary>Reasoning</summary>

各项修正的近精确抵消不是数值巧合，而是 Ward 恒等式约束多体修正在物理可观测量中组合方式的必然结果。
...
```mermaid
graph TD
    precursory_cooper_flow["前驱 Cooper 流方法 (0.79)"]:::external
    mu_vdiagmc_values["vDiagMC 赝势计算结果 (0.45)"]:::external
    dfpt_reliable_for_simple_metals["DFPT 对简单金属可靠 (0.63)"]:::external
    aluminum_parameters["铝的材料参数"]:::setting
    lithium_parameters["锂的材料参数"]:::setting
    magnesium_parameters["镁的材料参数"]:::setting
    sodium_parameters["钠的材料参数"]:::setting
    ab_initio_workflow["第一性原理 Tc 预测工作流 (0.20)"]:::derived
    tc_al_predicted["铝的 Tc 第一性原理预测 (0.18)"]:::derived
    tc_al_phenomenological["铝的 Tc 唯象预测 (0.85)"]:::orphan
    tc_li_predicted["锂的 Tc 第一性原理预测 (0.18)"]:::derived
    tc_li_phenomenological["锂的 Tc 唯象预测 (0.85)"]:::orphan
    tc_mg_na_near_qpt["镁和钠接近量子临界点 (0.17)"]:::derived
    al_pressure_transition["铝的压力诱导量子相变 (0.16)"]:::derived
    tc_improvement_over_phenomenological["Tc 预测精度数量级提升 (0.03)"]:::derived
...
📌 `ab_initio_workflow`   |   Belief: **0.20**
...
🔗 **noisy_and**([vDiagMC 赝势计算结果](#mu_vdiagmc_values), [DFPT 对简单金属可靠](#dfpt_reliable_for_simple_metals), [前驱 Cooper 流方法](#precursory_cooper_flow))

<details><summary>Reasoning</summary>

精确 $\mu^*$ + 可靠 $\lambda$ + PCF 方法 → 无可调参数的第一性原理工作流。
...
📌 `tc_al_predicted`   |   Belief: **0.18**

> 第一性原理工作流预测铝的超导转变温度为 $T_c^{\mathrm{EFT}} = 0.96$ K（Table II），与实验值 $T_c^{\mathrm{exp}} = 1.2$ K 偏差约 20%。

🔗 **noisy_and**([第一性原理 Tc 预测工作流](#ab_initio_workflow))


#### 铝的 Tc 唯象预测

📌 `tc_al_phenomenological`   |   Prior: 0.85   |   Belief: **0.85**
...
📌 `tc_li_phenomenological`   |   Prior: 0.85   |   Belief: **0.85**
...
📌 `al_pressure_transition`   |   Belief: **0.16**
...
📌 `tc_improvement_over_phenomenological`   |   Belief: **0.03**
...
🔗 **noisy_and**([铝的 Tc 第一性原理预测](#tc_al_predicted), [锂的 Tc 第一性原理预测](#tc_li_predicted))

<details><summary>Reasoning</summary>

铝（偏差 58%→20%）和锂（偏差 3 个数量级→1 个数量级）两个案例共同支撑总结论。
